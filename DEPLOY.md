...
```bash
# قاعدة البيانات ولوحة التحكم (مطلوبة)
DATABASE_URL=postgresql://USER:PASSWORD@localhost:5432/mmitweb
AUTH_SECRET=...               # ولّده: openssl rand -base64 32
AUTH_TRUST_HOST=true
ADMIN_EMAIL=admin@mmit.sa      # لإنشاء أول حساب عبر db:seed
...
## ٢. قاعدة البيانات + لوحة التحكم (CMS)

الموقع تُدار محتوياته بالكامل من لوحة تحكم تعتمد على PostgreSQL.

```bash
# 1) جهّز قاعدة بيانات PostgreSQL واضبط DATABASE_URL في .env
# 2) أنشئ الجداول
npm run db:migrate      # prisma migrate deploy
# 3) عبّئ المحتوى الأولي + أنشئ حساب المدير (من ADMIN_*)
npm run db:seed
```

- لوحة التحكم على المسار **`/admin`** (تسجيل الدخول `/admin/login`).
- بعد أول دخول **غيّر كلمة مرور المدير** من قسم «المستخدمون».
- ما يُدار من اللوحة: الإعدادات، الخدمات، المنتجات، الأعمال، الشركاء،
  الأسئلة الشائعة، المزايا، القطاعات، مراحل العمل، الصفحات، والمستخدمون.
- أي تعديل يظهر على الموقع فورًا.
- **رفع الصور**: تُحفظ في `public/uploads/` — تأكّد أن المجلد قابل للكتابة
  من المستخدم الذي يشغّل العملية (وأنه ضمن مجلد standalone المنسوخ).
