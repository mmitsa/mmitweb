...
export default async function PartnersAdmin() {
  ...
  return (
    <div>
      <AdminHeader title="الشركاء" description={`${partners.length} جهة`} addHref="/admin/partners/new" addLabel="إضافة شريك" />
        ...
        <table className="w-full text-right">
          <thead className="border-b border-outline-variant/20 bg-surface-container-low">
            <tr>
              ...
              <th className="px-5 py-3 font-head text-sm text-on-surface-variant">القطاع</th>
          ...
          <tbody>
            {partners.map((p) => (
              <tr key={p.id} className="border-b border-outline-variant/10 last:border-0 hover:bg-surface-container-low/50">
                ...
                <td className="px-5 py-3 text-sm text-on-surface-variant">{p.sector}</td>
                <td className="px-5 py-3">
                  <div className="flex items-center justify-end gap-1">
                    <Link href={`/admin/partners/${p.id}`} aria-label="تعديل" className="flex h-9 w-9 items-center justify-center rounded-lg text-on-surface-variant transition-colors hover:bg-surface-container hover:text-secondary">
            ...
            {partners.length === 0 && (
              <tr><td colSpan={3} className="px-5 py-10 text-center text-on-surface-variant">لا يوجد شركاء بعد.</td></tr>
            )}
          </tbody>
        </table>
      </div>
