...
export default async function SectorAdmin() {
  const rows = await prisma.sector.findMany({ orderBy: { order: "asc" } });
  return (
    <div>
      <AdminHeader title="القطاعات" description={`${rows.length} عنصر`} addHref="/admin/sectors/new" addLabel="إضافة" />
              ...
              <tr key={r.id} className="border-b border-outline-variant/10 last:border-0 hover:bg-surface-container-low/50">
                ...
                <td className="px-5 py-3">
                  <div className="flex items-center justify-end gap-1">
                    <Link href={`/admin/sectors/${r.id}`} aria-label="تعديل" className="flex h-9 w-9 items-center justify-center rounded-lg text-on-surface-variant transition-colors hover:bg-surface-container hover:text-secondary">
                      <Icon name="edit" className="text-[20px]" />
                    </Link>
                    <DeleteButton action={deleteSector.bind(null, r.id)} />
                  </div>
                </td>
              </tr>
            ))}
