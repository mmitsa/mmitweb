...
function StatItem({ stat, run }: { stat: Stat; run: boolean }) {
  ...
  return (
    <div className="group flex flex-col items-center text-center">
      <Icon name={stat.icon} className="mb-3 text-[34px] text-tertiary-fixed-dim transition-transform group-hover:scale-110" />
...
export function Stats({
  ...
  const stats: Stat[] = [
    { icon: "task_alt", target: projectCount, prefix: "+", label: "مشروع منجز" },
    { icon: "account_balance", target: clientCount, label: "جهات حكومية" },
    { icon: "apps", target: productCount, label: "أنظمة رقمية" },
...
export function ClientsStrip({ clients }: { clients: string[] }) {
  return (
    <section className="border-y border-outline-variant/30 bg-surface-container-low py-12">
      <Container>
        ...
        <div className="mt-8 text-center">
          <Link href="/partners" className="group inline-flex items-center gap-1.5 font-head text-sm font-medium text-secondary hover:text-on-secondary-fixed-variant">
            عرض كل الشركاء
